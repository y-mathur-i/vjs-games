document.addEventListener('DOMContentLoaded', () => {
    ...
    // chcek board // got the winning arrays from internet
    function checkBoard() {
        const winningArrays = [
        ...
        // taking four values in each array and pluggin in the board
        for (let y = 0; y < winningArrays.length; y++) {
            const square1 = squares[winningArrays[y][0]]
            const square2 = squares[winningArrays[y][1]]
            const square3 = squares[winningArrays[y][2]]
            const square4 = squares[winningArrays[y][3]]
            // now check if all have the class of player one
            if (square1.classList.contains('PlayerOne') &&
                ...
                // if yes we would like to display player one as winner
                result.innerHTML = "Player One Wins !!"
            } else if (square1.classList.contains('PlayerTwo') &&
